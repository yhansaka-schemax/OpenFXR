/**
 * Sampath Bank exchange rate scraper.
 * URL: https://www.sampath.lk/en/personal/foreign-exchange/tt-exchange-rates
 */

import { withBrowser } from '../utils/browser.js';
import { SOURCE_NAMES } from '../schema.js';

const URL = 'https://www.sampath.lk/en/personal/foreign-exchange/tt-exchange-rates';
...
export async function scrape() {
  try {
    ...
    if (!rates || Object.keys(rates).length === 0) {
      console.error('[sampath] No rates parsed'); return { name: SOURCE_NAMES.SAMPATH, rates: null };
    }
    console.log(`[sampath] Scraped ${Object.keys(rates).length} currencies`);
    return { name: SOURCE_NAMES.SAMPATH, rates };
  } catch (err) {
    console.error('[sampath] Error:', err.message);
    return { name: SOURCE_NAMES.SAMPATH, rates: null };
  }
}
