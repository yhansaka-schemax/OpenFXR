/**
 * Hatton National Bank (HNB) exchange rate scraper.
...
const URL = 'https://www.hnb.net/exchange-rates';
...
export async function scrape() {
  try {
    ...
    if (!rates || Object.keys(rates).length === 0) {
      console.error('[hnb] No rates parsed'); return { name: SOURCE_NAMES.HNB, rates: null };
    }
    console.log(`[hnb] Scraped ${Object.keys(rates).length} currencies`);
    return { name: SOURCE_NAMES.HNB, rates };
  } catch (err) {
    console.error('[hnb] Error:', err.message);
    return { name: SOURCE_NAMES.HNB, rates: null };
  }
}
